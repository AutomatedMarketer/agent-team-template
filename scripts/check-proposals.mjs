...
if (summary.unpriced && summary.proposed > 0) {
...
for (const row of listOrEmpty(written.proposals) ?? []) {
...
if (summary.gaps) {
  ...
  for (const gap of listOrEmpty(written.gaps) ?? []) {
...
// Zero proposals is a real answer, not an empty one, and it is the ordinary answer for anyone
// whose week is not the week this team was built for. Printing "every line above quotes you" over
// an empty list reads like something went wrong, which is the opposite of true: the gaps ARE the
// output, and the lesson already says so. The closing line should agree with the lesson.
if (summary.proposed === 0) {
  console.log('\nNothing on the team matched your week. That is a real answer, not a failed run -')
  console.log('the gaps above are the specification for what gets built next.')
} else {
